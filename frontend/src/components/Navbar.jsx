...
function Navbar({ currentPage, setCurrentPage, language, t, toggleLanguage }) {
  ...
  const navItems = [
    { id:'home', label: t.nav.home, icon:'fa-house' },
    { id:'times', label: t.nav.teams, icon:'fa-shield-halved' },
    { id:'nacoes', label: t.nav.nations, icon:'fa-earth-americas' },
    { id:'pesquisa', label: t.nav.search, icon:'fa-magnifying-glass' },
    { id:'editar', label: t.nav.edit, icon:'fa-pen-to-square' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <div className="flex items-center gap-2">
            {/* Language Toggle */}
            <button onClick={toggleLanguage} className={`p-2.5 rounded-xl transition-all duration-300 hover:scale-110 ${scrolled ? 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800' : 'text-white/80 hover:bg-white/10'}`} title={language === 'pt' ? 'Switch to English' : 'Mudar para Português'}>
